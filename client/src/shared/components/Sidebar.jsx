...
function Sidebar({ onClose }) {
  ...
  return (
    <aside className="flex h-full flex-col border-r border-stone-800/50 bg-[#1b2127]/95 text-stone-400 backdrop-blur-xl">
      {/* Brand Section */}
      <Link
        to="/home"
        onClick={handleNavClick}
        className="flex h-24 cursor-pointer items-center gap-4 border-b border-stone-800/50 px-8 transition-colors hover:bg-white/5"
      >
        <div className="relative">
          <motion.img
            ...
            src={Logo}
            alt="Logo"
            className="h-12 w-12 rounded-2xl object-cover shadow-2xl ring-2 shadow-blue-500/10 ring-blue-500/20"
            crossOrigin="anonymous"
            referrerPolicy="no-referrer"
          />
          <div className="absolute -right-1 -bottom-1 h-4 w-4 rounded-full border-2 border-[#1b2127] bg-green-500" />
        </div>
      ...
      <nav className="custom-scrollbar flex-1 space-y-2 overflow-y-auto px-4 py-8">
        <div className="mb-4 px-4 text-[10px] font-bold tracking-widest text-stone-600 uppercase">
          Main Menu
        </div>
        {navItems.map((item) => (
          <NavLink
          ...
          >
            {({ isActive }) => (
              <>
                <span
                  className={`transition-colors duration-300 ${isActive ? 'text-blue-500' : 'text-stone-500 group-hover:text-blue-400'}`}
                >
                  {item.icon}
                </span>
