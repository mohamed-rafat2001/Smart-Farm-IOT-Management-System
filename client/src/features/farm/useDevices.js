...
function useDevices(firebaseUrl, enableNotifications = false) {
  ...
  useEffect(() => {
    ...
    if (enableNotifications && Devices && lastData && lastData._url === firebaseUrl) {
      Object.keys(Devices).forEach(deviceName => {
        ...
        if (prevDevice) {
          ...
          const currentStatus = String(device.Status || device.status || 'Active').toLowerCase();
          const prevStatus = String(prevDevice.Status || prevDevice.status || 'Active').toLowerCase();

          // 1. Detect Status Change
          if (currentStatus !== prevStatus) {
            addNotification({
              type: currentStatus === 'online' || currentStatus === 'active' ? 'success' : 'critical',
              title: 'Device Status',
          ...
          const keysToIgnore = ['Status', 'status', 'last_seen', 'updated_at', 'Auto_change'];
